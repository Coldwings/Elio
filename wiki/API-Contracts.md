...
| `io::async_recv()` and `io::async_send()` | Submit one socket receive/send request and report actual transfer or errno. Cancellable overloads honor the provided token at the documented wait points. | Keep the socket fd and buffer alive until completion. Treat short socket I/O and zero-length peer close according to protocol needs. |
