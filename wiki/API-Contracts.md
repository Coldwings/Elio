...
| `net::uds_connect()` | Converts the supplied UDS address, attempts the connection, and reports socket/connect failures as `std::nullopt` with `errno` set from the failing operation. | Ensure the address meets `unix_address` length rules and represents an intended filesystem or abstract namespace target. Check the optional result before use. |
| `net::uds_stream` | Provides stream I/O semantics matching TCP stream base/exact helpers over Unix-domain sockets, including per-call `SIGPIPE` suppression where supported for socket writes. | Apply the same short I/O and buffer-lifetime rules as TCP streams. |
