...
// Regression tests for: send() must report success when its value is delivered
// to a receiver via a direct handoff (the receiver steals from a blocked
// sender), even if the channel is closed before the sender resumes. Without
// setting sender->success_ = true in the steal paths, send() falls back to
// `!closed_` and incorrectly returns false once the channel is closed.
//
// To make the "close before the sender resumes" window deterministic for the
// recv() paths, a single-threaded scheduler is used and the steal + close are
// performed inside one coroutine: schedule_handle() enqueues the resumed sender
// onto the only worker, so it cannot run until the stealer coroutine yields,
// guaranteeing close() wins the race.
