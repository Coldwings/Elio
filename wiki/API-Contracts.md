...
| `runtime::scheduler` | Starts worker threads, runs submitted coroutine tasks, routes per-worker I/O, and performs orderly shutdown according to its API. Raw `spawn` detaches construction-time ancestry for independent work; raw `schedule` preserves await ancestry when resuming suspended work. | Start it before submitting work that requires runtime or I/O services. Keep it alive while scheduler-owned objects and tasks can use it. Do not use `spawn` to resume a suspended handle or `schedule` for initial ownership handoff. |
