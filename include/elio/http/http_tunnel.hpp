...
namespace detail { struct tunnel_stream_access; }
...
public:
    ...
    coro::task<io::io_result> read(void* buffer, size_t length, coro::cancel_token token = {}) {
        ...
        try {
            ...
            if (result.result < 0) fail(-result.result);
            co_return result;
    ...
    // Meaningful after a zero read; never infers EOF from a failed read.
    net::close_scope read_end_scope() const { return functions_->read_scope(object_); }

    coro::task<tunnel_write_result> write(const void* data, size_t size,
    ...
    coro::task<tunnel_write_result> writev(std::span<const iovec> parts,
                                          coro::cancel_token token = {}) {
