...
|-----------|-----------------|-----------------------|
| `coro::task<T>` | A task is a move-only, single-shot lazy owner. Moving transfers the unstarted coroutine frame and leaves the source empty. Awaiting binds virtual-stack ancestry to the actual awaiter, then returns the result or rethrows the stored exception. Destroying a non-empty task destroys its frame unless ownership was transferred to the runtime. Runtime policy remains in the promise's shared execution context, not in the task object. | Do not await an empty or already-awaited task. Do not treat moving a task as migration or cancellation of running work. Keep objects referenced by the coroutine alive across suspension points. |
...
| `join_handle<T>` | Awaiting a join handle returns the spawned task result or rethrows its exception. `is_ready()` is a non-blocking readiness check. Its state shares the spawned wrapper promise's execution context without retaining or addressing the coroutine frame. | Await or intentionally discard join handles according to the task lifetime you need. Do not use a moved-from handle. Sharing an execution context does not make frame access valid after completion. |
...
| `runtime::scheduler` | Starts worker threads, runs submitted coroutine tasks, routes per-worker I/O, and performs orderly shutdown according to its API. Raw `spawn`/`spawn_to` detach construction-time ancestry for independent work. Raw `try_schedule`/`try_schedule_to` preserve await ancestry and never consume a borrowed suspended handle when enqueue is rejected. | Start it before submitting work that requires runtime or I/O services. Keep it alive while scheduler-owned objects and tasks can use it. Do not use a `spawn` variant to resume a suspended handle or a `try_schedule` variant for initial ownership handoff. Check a `try_schedule` result and retain, resume, or otherwise resolve the still-live handle on failure. |
...
| `coro::task_execution_context`, `promise_base::execution_context()` | Keep scheduler-visible task policy alive through shared ownership independently of coroutine-frame lifetime. The current context stores user affinity and the internal worker-local flag; it does not own the frame or an awaitable's operation state. | Treat the context as a runtime integration boundary, not as proof that a coroutine frame or pending operation remains live. Do not store raw promise pointers in external owners. |
| `coro::promise_base::affinity()`, `set_affinity()`, `clear_affinity()` | Store and report caller-requested affinity through the shared execution context. Scheduler placement uses the context's effective-affinity boundary. | Treat them as coroutine-runtime integration points, not as general synchronization APIs. |
...
| `net::uds_stream` | Provides stream I/O semantics matching TCP stream base/exact helpers over Unix-domain sockets, including cancellation-aware read/write helpers and per-call `SIGPIPE` suppression where supported for socket writes. | Apply the same short I/O, cancellation, and buffer-lifetime rules as TCP streams. |
...
| `io::io_op`, `io::io_request`, and `io::io_backend` | Define the low-level backend request/result contract used by concrete I/O backends. | Treat them as backend integration interfaces. Keep request-owned buffers, addresses, and op state alive according to the backend contract. |
| `io::io_uring_backend` and `io::epoll_backend` | Normalize supported backend completions into Elio awaitable results. | Select a backend supported by the kernel and deployment. Do not bypass backend ownership rules. |
...
| `rpc::rpc_server<Stream>` | Dispatches registered handlers for valid method frames and enforces configured session, per-session in-flight request, and timeout limits. | Assign stable unique method IDs, maintain schema compatibility, validate caller identity/authorization, and keep handler captures alive. |
...
| `rdma::dispatcher` and `rdma::cq_pump` | Deliver completions to awaiting operations according to wr_id/op_state lifecycle rules. | Drive the dispatcher/cq pump until all outstanding operations complete or are intentionally drained. |
