...
|-----------|-----------------|-----------------------|
| `coro::task<T>` | A task is a move-only, single-shot lazy owner. Moving transfers the unstarted coroutine frame and leaves the source empty. Awaiting binds virtual-stack ancestry to the actual awaiter, then returns the result or rethrows the stored exception. Destroying a non-empty task destroys its frame unless ownership was transferred to the runtime. | Do not await an empty or already-awaited task. Do not treat moving a task as migration or cancellation of running work. Keep objects referenced by the coroutine alive across suspension points. |
